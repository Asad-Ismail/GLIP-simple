...
class GLIPHead(nn.Module):
    def __init__(self, in_channels=256,num_classes=80):
        ...
        self.scales = nn.ModuleList([Scale(init_value=1.0) for _ in range(4)])  # Assuming 4 feature levels
        ...
        self.log_scale = nn.Parameter(torch.Tensor([0.0]), requires_grad=True)  # Initialize with log(1) = 0
        self.bias_lang = nn.Parameter(torch.zeros(lang_dim), requires_grad=True)
        self.bias0 = nn.Parameter(torch.Tensor([0.0]), requires_grad=True)
